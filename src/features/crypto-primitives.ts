...
export function hmac_sha512(initKey: Buffer, inputs: Buffer[]) {
...
export function chacha20poly1305Encrypt(
  ...
  const resultCode = Module._emscripten_chacha20poly1305_enc(
    keyArrPtr,
    nonceArrPtr,
    inputArrPtr,
    inputLen,
    outputArrPtr,
    outputArrPtr + inputLen,
    tagLen,
    1
  );
...
export function sha3_256(input: Buffer) {
  validateBuffer(input);
  const inputLen = input.length;
  const inputArrPtr = Module._malloc(inputLen);
  const inputArr = new Uint8Array(Module.HEAPU8.buffer, inputArrPtr, inputLen);
...
export function cardanoMemoryCombine(input, password) {
  validateBuffer(input);
  ...
  const transformedPasswordArr = new Uint8Array(
    Module.HEAPU8.buffer,
    transformedPasswordArrPtr,
    transformedPasswordLen
  );
  ...
  Module._emscripten_cardano_memory_combine(
    transformedPasswordArrPtr,
    transformedPasswordLen,
    inputArrPtr,
    outputArrPtr,
