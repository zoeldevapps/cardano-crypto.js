...
export async function decodePaperWalletMnemonic(paperWalletMnemonic: string) {
  ...
  const mnemonicScrambledPart = paperWalletMnemonicAsList
    .slice(0, 18)
    .join(" ");
  const mnemonicPassphrasePart = paperWalletMnemonicAsList
    .slice(18, 27)
    .join(" ");

  const passphrase = await mnemonicToPaperWalletPassphrase(
    mnemonicPassphrasePart
  );
  const unscrambledMnemonic = await paperWalletUnscrambleStrings(
    passphrase,
    mnemonicScrambledPart
  );

  return unscrambledMnemonic;
}

async function mnemonicToPaperWalletPassphrase(
  mnemonic: string,
  password?: string
): Promise<string> {
  ...
  return (
    await pbkdf2(mnemonicBuffer, saltBuffer, 2048, 32, "sha512")
  ).toString("hex");
}
...
async function paperWalletUnscrambleStrings(passphrase, mnemonic) {
  ...
  const output = await pbkdf2(
    passphrase,
    input.slice(0, saltLength),
    10000,
    outputLength,
    "sha512"
  );
